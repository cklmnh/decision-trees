...
def buildTree (instances, attributes, confidence, target_attr, parent_attr_val = '', depth = 0):
    ...
    if initial_entropy == 0.0:
        ...
        best_attr, freq_val = getBestAttributeAndMostFreqVal(instances, attributes, confidence, target_attr, initial_entropy)
        if best_attr is not None:
            print "Best attribute at depth %d: " % depth, best_attr.name
            ...
            for attr_val in best_attr.values:
                ...
                node.children.append(buildTree(subset, new_attributes, confidence, target_attr, attr_val, depth + 1))
            return node
        else:
            return Node(target_attr.name, [], target_val, target_val, parent_attr_val, depth)
        
def getBestAttributeAndMostFreqVal(instances, attributes, confidence, target_attr, initial_entropy):
    max_gain = 0.0
